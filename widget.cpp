...
if(ninit()){
trace("error in avcoded iniitalization, exiting")
exit(-1);
}
ac.volume=40;
audio=video=0;
...
void widget::pushframe(){
int ret=0;
//trace("widget::pushframe,tweak,framecount,audiosamplecount,video,audio"<<tweak<<" "<<framecount<<" "<<audiosamplecount<<"video"<<video<<"audio"<<audio)
...
//trace("diff in frame"<<(float)(((int64_t)ac.videopts-((int64_t)ac.videostreampts*(int64_t)ac.audiopts)/(int64_t)ac.audiostreampts)*(int64_t)ac.frameratenum)/((int64_t)ac.framerateden*(int64_t)ac.videostreampts))
...
//trace("MORE videopts"<<ac.videopts<<" "<<ac.videostreampts<<" "<<ac.audiopts<<" "<<ac.audiostreampts<<" "<<((int64_t)ac.videopts-((int64_t)ac.videostreampts*(int64_t)ac.audiopts)/(int64_t)ac.audiostreampts))
...
//trace("LESS videopts"<<ac.videopts<<" "<<ac.videostreampts<<" "<<ac.audiopts<<" "<<ac.audiostreampts<<" "<<((int64_t)ac.videopts-((int64_t)ac.videostreampts*(int64_t)ac.audiopts)/(int64_t)ac.audiostreampts))
...
//trace("~widget::pushframe"<<framecount<<" "<<audiosamplecount)
}
void widget::paintEvent(QPaintEvent*){
//trace("widget::paintEvent()")
QPainter p(this);
...
void widget::slotclicked(const QString& fn){
trace("widget::slotclicked")
static QString prevfilename="/temp/dump";
QString filename;
if((filename=fn).isEmpty())
filename=QFileDialog::getOpenFileName(this,"file name",prevfilename,"Files (*.mp4 *.mp3 *.avi *.flv *.dat *.3gpp *.wmv *.webm);; All (*.*)");
...
void widget::slotsldrvaluechanged(int val){
//trace("widget::slotclicked,sldr"<<val<<" "<<seekfile<<" "<<pause)
...
ac.volume=val;
seekfile=true;
//trace("~widget::slotsldrvaluechanged()")
}
void widget::enterEvent(QEvent *e){
trace("widget::enterEvent")
...
trace("widget::mousePressEvent"<<pause<<"a"<<audio<<" "<<video)
pause=!pause;
if(pause){
if(audio)
SDL_PauseAudio(1);
...
trace("~widget::mousePressEvent")
}
void widget::mouseDoubleClickEvent(QMouseEvent *e){
trace("widget::mouseDoubleClickEvent")
